...
class LaneFinder {
    ...
    private fun drawLane(image: Mat, windowsLeft: ArrayList<Window>, windowsRight: ArrayList<Window>) {
        ...
        lineLeft.sortByDescending { point -> point.y }

        val lineMiddle = ArrayList<Point>()

        Log.d(javaClass.simpleName, "right size: ${lineRight.size}" + " left size: ${lineLeft.size}" )
        for (i in 0..lineRight.indices.endInclusive) {
            if (lineLeft.size > i) {
            val middlePointX = (lineRight[i].x + lineLeft[i].x) / 2
            val middlePointY = (lineRight[i].y + lineLeft[i].y) / 2
            lineMiddle.add(Point(middlePointX, middlePointY))
            }
        }
        ...
        val middlePoints = MatOfPoint(*(lineMiddle).toTypedArray())
        val list = mutableListOf<MatOfPoint>()
        list.add(middlePoints)
        Imgproc.polylines(image, list, false, Scalar(255.0, 0.0, 0.0), 3)
//        Imgproc.rectangle(
//                image, lineMiddle.first(), lineMiddle.last(), Scalar(0.0, 0.0, 255.0))
