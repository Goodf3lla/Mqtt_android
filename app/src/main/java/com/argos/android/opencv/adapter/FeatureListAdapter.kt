...
class FeatureListAdapter(private val features: ArrayList<Feature>, context: Context) : RecyclerView.Adapter<FeatureListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.pictureButton.setOnClickListener {
            mainActivityCallback.chooseImage(features[position])
        }

        if (features[position].featureName.equals(Feature.LANE_KEEPING)) {
            holder.pictureButton.visibility = View.GONE
        }
